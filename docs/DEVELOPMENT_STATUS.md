...
- Latest verified checkpoint: MLB editorial preview/recap card integration
...
| Trivia/lore provenance | Active lore inventory: 96 retained entries; 41 verified, 55 reviewed-unverified, 96 unique IDs, 0 missing/invalid HTTPS URLs. Verified IDs are listed below; retained legacy claims remain explicitly unverified, including the contradicted Pedro Martínez item. The latest production batch adds six source-reviewed records spanning records, baseball origins, and ballpark history. |
| Trivia/lore rotation | Passive wallboard rotation filters to the 41 verified base lore entries plus four verified historical biographies (45 current runtime learning items), uses a seeded shuffle, preserves lore state while the parent slide is hidden, makes the inactive slide inert, advances only while the combined learning stream is visible, covers the full available pool before reshuffling, prevents the prior pool's final item from opening the next pool, and renders no lore cards when the verified pool is empty. |
...
- `record-orel-hershiser-59-scoreless-1988` — https://www.mlb.com/news/orel-hershiser-scoreless-inning-streak-record
- `record-fernando-tatis-two-grand-slams-inning` — https://www.mlb.com/news/fernando-tatis-hit-2-grand-slams-in-1-inning
- `record-jim-abbott-one-handed-no-hitter-1993` — https://www.mlb.com/news/jim-abbott-no-hitter-streaming-on-mlb
...
- `record-rennie-stennett-seven-for-seven-1975` — https://baseballhall.org/discover/inside-pitch/stennett-goes-seven-for-seven
- `record-twins-two-triple-plays-1990` — https://www.mlb.com/twins/history/timeline-1990s
- `record-harrah-wills-back-to-back-inside-park-1977` — https://www.mlb.com/news/mlb-records-that-have-only-happened-once/c-45942126
- `history-first-national-association-game-1871` — https://www.retrosheet.org/1stGame.htm
- `history-first-al-nl-night-game-1935` — https://www.mlb.com/news/first-night-game-in-al-nl-history
- `history-comiskey-experimental-night-game-1910` — https://sabr.org/gamesproj/game/august-27-1910-comiskey-park-hosts-its-first-night-baseball-game
...
| Schedule date semantics | Before the first current-day game starts, the passive Scoreboard may retain eligible prior-day final/carryover games alongside today’s slate. Once any current-day game has started, including reaching Final status, prior-day scores and completed games are removed; only the current-day slate remains. Overlapping game IDs are deduplicated in favor of the current-day object. |
...
| Wallboard view navigation | The wallboard header presents Scoreboard, Game Feed, AL Standings, and NL Standings as keyboard-accessible clickable tabs. Outer view auto-rotation is disabled for testing; Game Feed’s internal game rotation and card editorial rotations remain available through the inner rotation control. Schedule, Statcast, Who's Hot, and Sports remain selectable manual destinations. |
| Wallboard scoreboard density | Before any current-day game starts, the Scoreboard presents eligible prior-day carryover plus today’s slate. Once any current-day game has started, including a completed game, it filters to the current-day slate and never reintroduces prior-day scores. Fixed viewport-fitting card slots prevent scoreboard scrolling or clipping; larger filtered slates rotate one card slot at a time. |
| Wallboard scoreboard card refinement | Scoreboard cards use team-primary shading with club-specific secondary/accent gradients, centered scheduled start times, and provider-safe pitcher labels. Live and pre-game team panels use the same larger fixed height. Live cards omit the redundant `Live: see Game Feed` panel text, show provider-sourced pitcher/hitter names, three out indicators beneath `AT`, the live pitching count (`balls-strikes`) beneath the outs, and a centered three-base runner diamond (1B/2B/3B only); When live and completed games exceed the visible slot count, the bottom row becomes a completed-game rotation band and every remaining slot rotates only through live games. |
| Wallboard learning integration | The global lower Scoreboard rectangle is disabled. Each upcoming or completed game card owns a fixed 48px editorial strip. The server-side `/api/game/:gamePk/editorial` adapter accepts usable MLB recap headline/blurb/URL data and preview-story prose or up to three distinct accessibility descriptions from official DAPI media parts; absent or malformed editorial content remains unavailable. When official content and a relevant hot hitter/pitcher both exist, the card alternates between them on the existing 9.2-second wallboard cadence; otherwise it falls back to hot performers, verified Lore & Curios content, or a biography. Article content is one clickable region with no extra per-card link or attribution controls. A compact page-level MLB source note appears in the footer. |
| Wallboard Game Feed rotation | Game Feed rotates through current-day active games first and current-day completed games once current-day play exists; prior-day carryover finals are excluded once any current-day game has started, including after that game becomes Final. Before any current-day game starts, eligible carryover finals remain available. Upcoming games are excluded and Scoreboard selection remains static. |
...
| Game Feed overview scope | When current-day games have started, the wallboard Game Feed overview includes current-day active and completed games; prior-day carryover finals are excluded after the first current-day start and do not return when a current-day game becomes Final. Before any current-day game starts, eligible displayed-slate completed games remain available. |
| Provider play fallback | Schedule no longer invents play, pitching, lineup, or venue commentary when the provider omits play-by-play; it displays an explicit unavailable state instead. |
| Scoreboard keyboard access | Scoreboard game cards retain click selection without selection highlighting and now expose button semantics, labels, focus styling, and Enter/Space activation. |
...
| News/headline sources | `/api/news` polls official MLB.com RSS as primary and ESPN's documented MLB RSS as a secondary source server-side. Articles preserve publisher attribution, original URLs, timestamps, and feed-provided summaries; the UI does not copy full articles or render feed images. |
