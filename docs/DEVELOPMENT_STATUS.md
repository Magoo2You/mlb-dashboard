...
| Trivia/lore rotation | Passive wallboard rotation filters to the 30 verified base lore entries plus verified historical biographies (34 current runtime learning items), uses a deterministic seeded shuffle, covers the full available pool before reshuffling, prevents the prior pool's final item from opening the next pool, and renders no lore cards when the verified pool is empty. |
