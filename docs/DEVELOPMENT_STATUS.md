...
| Trivia/lore provenance | Active lore inventory: 85 retained entries; 30 verified, 55 reviewed-unverified, 85 unique IDs, 0 missing/invalid HTTPS URLs. Verified IDs are listed below; retained legacy claims remain explicitly unverified, including the contradicted Pedro Martínez item. The latest production batch adds six source-reviewed records spanning pitching history, unusual single-game achievements, and Jewish baseball history. |
| Trivia/lore rotation | Passive wallboard rotation filters to the 30 verified base lore entries plus verified historical biographies (34 current runtime learning items), uses a seeded shuffle, preserves lore state while the parent slide is hidden, makes the inactive slide inert, advances only while the Scoreboard and Lore & Curios subcard are visible, covers the full available pool before reshuffling, prevents the prior pool's final item from opening the next pool, and renders no lore cards when the verified pool is empty. |
...
| Wallboard Game Feed | Game Feed uses provider team logos as subtle visual identity and reserves its former runner-only region for up to five newest-first provider-backed scoring or notable plays when available; it retains the runner diamond when no qualifying play is present. |
...
| Scoreboard selection styling | Scoreboard cards remain clickable for Game Feed selection but no longer highlight the initially selected/first game. |
