...
| Schedule date semantics | The passive Scoreboard evaluates the current local calendar time and date. Before the first local-day game starts, it shows prior-day final/carryover games alongside the current day’s scheduled slate; once a current-day game starts, it shows only that day’s scheduled, live, and final games. It never substitutes a future slate after the current day completes. |
...
| Wallboard scoreboard density | The Scoreboard tab renders the complete provider-returned local-day slate in a centered responsive grid with no pagination, selected-game rotation, or scoreboard-owned scrolling. It uses the local-day rollover policy above and presents yesterday/overnight carryover first, followed by today's ongoing, upcoming, and completed sections. |
| Wallboard learning integration | When the complete slate fits the Scoreboard’s fixed canvas, the remaining space contains a compact rotating pair of provider-backed Headlines, Hot Hitters, or verified Lore & Curios cards. A crowded slate remains games-only rather than shrinking or scrolling the cards. |
| Wallboard Game Feed rotation | Game Feed rotates through active games first, then completed games in the currently displayed slate (including overnight carryover) while its wallboard slide is visible; upcoming games are excluded and Scoreboard selection remains static. |
...
| Game Feed overview scope | When live games exist, the wallboard Game Feed overview includes all active games plus all completed games in the currently displayed slate, including overnight carryover; when none are live, it includes the displayed slate's completed games. |
