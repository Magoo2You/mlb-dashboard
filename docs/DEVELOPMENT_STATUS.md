...
| Trivia/lore rotation | Passive wallboard rotation filters to the 30 verified base lore entries plus verified historical biographies (34 current runtime learning items), uses a seeded shuffle, preserves lore state while the parent slide is hidden, makes the inactive slide inert, and advances the consumed page only while the Scoreboard/Game Feed slide and Lore & Curios tab are visible, covers the full available pool before reshuffling, prevents the prior pool's final item from opening the next pool, and renders no lore cards when the verified pool is empty. |
...
| Wallboard view navigation | The wallboard header now separates Scoreboard, Game Feed, AL Standings, and NL Standings into distinct rotation tabs. Schedule, Statcast, Who's Hot, and Sports remain selectable manual destinations; only the four primary wallboard tabs auto-rotate, and Sports is never auto-rotated. |
