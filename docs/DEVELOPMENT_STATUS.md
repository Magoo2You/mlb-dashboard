...
| Wallboard Game Feed rotation | Game Feed rotates through eligible live and completed games while its wallboard slide is visible; Scoreboard selection remains static and does not rotate. |
...
| Game Feed overview scope | When live games exist, the wallboard Game Feed overview includes all active games plus all completed games from the current local day; when none are live, it includes the current-day completed games. |
