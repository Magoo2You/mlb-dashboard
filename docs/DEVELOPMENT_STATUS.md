...
| Schedule date semantics | The passive Scoreboard always retains eligible prior-day final/carryover games before the current day’s scheduled, live, and final games, including after a current-day game starts. It deduplicates overlapping game IDs in favor of the current-day object and never substitutes a future slate after the current day completes. |
...
| Wallboard scoreboard card refinement | Scoreboard game cards use a taller, wider team matchup layout with larger team logos, team-primary-color shading with club-specific secondary/accent gradients (including provider abbreviation aliases and a neutral fallback), centered first-pitch times for explicitly scheduled games, and transparent provider-sourced pitcher labels: projected names appear only for scheduled games; completed cards show the verified winning or losing decision pitcher on the corresponding team panel when available and otherwise show final pitcher data as unavailable; live cards identify pitcher data as available in the feed. |
| Wallboard learning integration | When the displayed slate contains one or fewer games, the remaining space contains a compact rotating pair of provider-backed Headlines, Hot Hitters, or verified Lore & Curios cards. Larger slates use fixed viewport-fitting scoreboard slots and card-by-card rotation rather than scrolling or clipping the cards. |
...
| Wallboard pause semantics | The wallboard pause control freezes the primary four-view rotation and the Headlines/Hot Hitters/Lore inner rotations together. Scoreboard games do not rotate. |
