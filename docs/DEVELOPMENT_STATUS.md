...
| Trivia/lore provenance | Active lore inventory: 90 retained entries; 35 verified, 55 reviewed-unverified, 90 unique IDs, 0 missing/invalid HTTPS URLs. Verified IDs are listed below; retained legacy claims remain explicitly unverified, including the contradicted Pedro Martínez item. The latest production batch adds five source-reviewed records spanning pitching history, a baseball oddity, and Jewish baseball history/biography. |
| Trivia/lore rotation | Passive wallboard rotation filters to the 35 verified base lore entries plus four verified historical biographies (39 current runtime learning items), uses a seeded shuffle, preserves lore state while the parent slide is hidden, makes the inactive slide inert, advances only while the combined learning stream is visible, covers the full available pool before reshuffling, prevents the prior pool's final item from opening the next pool, and renders no lore cards when the verified pool is empty. |
