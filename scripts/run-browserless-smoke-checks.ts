...
function runScrollOwnershipChecks(): void {
  ...
  assert.match(passiveScheduleComponent, /h-\[150px\] max-h-\[150px\] overflow-hidden rounded-lg border px-2\.5 py-2 text-center/);
  assert.match(passiveScheduleComponent, /h-\[280px\]/);
  assert.match(passiveScheduleComponent, /Array\.from\(\{ length: 3 \}/);
