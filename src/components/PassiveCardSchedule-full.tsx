...
export const PassiveCardSchedule: React.FC<PassiveCardScheduleProps> = ({
  ...
  // Shuffle Baseball Lore items randomly on each rotation for variety - RANDOM SHUFFLING EVERY 2ND ROTATION
  useEffect(() => {
    if (BASEBALL_LORE_ITEMS.length <= 3) return;
    
    const interval = setInterval(() => {
      // Randomize order of lore items on each rotation - gives random shuffle effect every other cycle!
